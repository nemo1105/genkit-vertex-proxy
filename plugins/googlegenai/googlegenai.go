...
type VertexAI struct {
	...
	// CredentialsJSON, if provided, overrides ADC and CredentialsFile.
	CredentialsJSON []byte
	// CredentialsFile, if provided, overrides ADC; ignored when CredentialsJSON is set.
	CredentialsFile string
	// ProxyURL applies only to the Vertex client created by this plugin.
	ProxyURL string
...
func (v *VertexAI) Init(ctx context.Context) []api.Action {
	...
	if len(v.CredentialsJSON) > 0 {
		...
		detectOpts.CredentialsFile = v.CredentialsFile
	}

	cred, err := credentials.DetectDefault(detectOpts)
	if err != nil {
	...
	httpClient, err := httptransport.NewClient(&httptransport.Options{
		Credentials:      cred,
		BaseRoundTripper: otelhttp.NewTransport(baseTransport),
		Headers: http.Header{
